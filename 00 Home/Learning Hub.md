...
> This hub is now maintained as a Base: [[00 Home/Learning Hub.base|Learning Hub Base]].

## Collections

- 👁️ **[[01.1 Computer Vision — Foundations Study Tracker.base|Computer Vision Foundations Base]]**
- 🧠 **[[01.2 DL Daily Lessons — Step-by-Step + Quiz.base|DL Daily Lessons Base]]**
- 📝 **[[01.3 DL Homework Practice — MIT 6.7960.base|MIT 6.7960 Homework Base]]**
...
const rows = [
  {
    ...
    pages: dv.pages('"01.1 Computer Vision — Foundations Study Tracker"').where(p => p.file.ext === "md"),
  ...
  {
    area: "🧠 DL Lessons",
    pages: dv.pages('"01.2 DL Daily Lessons — Step-by-Step + Quiz"').where(p => p.file.ext === "md"),
  ...
  {
    area: "📝 DL Homework",
    pages: dv.pages('"01.3 DL Homework Practice — MIT 6.7960"').where(p => p.file.ext === "md"),
    done: p => p.Status === "Done",
    active: p => p.Status === "In-Progress",
    todo: p => p.Status === "Not-Started"
  }
...
```dataviewjs
const rows = dv.pages('"01.1 Computer Vision — Foundations Study Tracker"')
...
```dataviewjs
const rows = dv.pages('"01.2 DL Daily Lessons — Step-by-Step + Quiz"')
  .where(p => p.file.ext === "md")
...
```dataviewjs
const rows = dv.pages('"01.3 DL Homework Practice — MIT 6.7960"')
  .where(p => p.file.ext === "md")
  .sort(p => p["HW #"] ?? 999, 'asc')
  .map(p => [
    p.file.link,
