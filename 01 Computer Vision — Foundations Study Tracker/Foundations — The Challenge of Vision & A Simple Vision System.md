...
## 📎 Supplementary resources for this block (added 2026-07-17)

Day 1 (Ch 1–4) is a motivational/foundational block and is **not** in the per-day external-resource map (that mapping begins at Day 2), so there is no mapped adensur folder or dedicated Udemy module to fetch here. For orientation, the closest conceptual anchor is the **Udemy "Intro" module** ([Mastering Computer Vision — pixels, image scaling/interpolation](https://www.udemy.com/course/mastering-computer-vision-from-pixel-to-detection-to-gen-cv/)): it grounds the very first idea of Ch 1 — that an image is *just a grid of intensity samples* carrying no explicit scene labels — which is exactly what makes the inverse problem $\hat S=\arg\max_S p(I\mid S)p(S)$ ill-posed. The **adensur "Computer Vision Zero to Hero"** repo ([index](https://github.com/adensur/blog/tree/main/computer_vision_zero_to_hero)) starts its mapped material at Day 3 (`00_introduction`, `01_simple_classification`), so it is previewed here only as the natural next step once learning begins.

Net: nothing new to fetch/integrate for Day 1 beyond the four chapters themselves; the mapped resources kick in from Day 2 (image formation → pixels/interpolation) onward.

---

## 🆕 Latest CV Research — 2026-07-17

A fresh scan (top venues + newest arXiv `cs.CV`), prioritizing items connected to today's foundations themes — inverse inference under a prior (Ch 1), the pixels→edges→surfaces representation cascade (Ch 2), perceptual reading of images (Ch 3), and fairness (Ch 4). These are distinct from the 2026-07-01 digest above.

**1. Vision Pretraining for Dense Spatial Perception** — (arXiv:2607.05247, 6 Jul 2026, cs.CV). Argues that modern visual foundation models optimize *semantic invariance* and thereby throw away *spatial* structure, then proposes a **boundary-centric** pretraining objective that recovers structured, metric, actionable representations directly from pixels. *Why it matters:* a live restatement of Marr's warning against collapsing "the task" into one net — it explicitly re-injects the Ch 2 idea that boundaries/edges are the scaffold from which surface geometry is propagated. [arXiv](https://arxiv.org/abs/2607.05247)

**2. VLRC: Vision-Language Reprojection Consistency as a Scalable Signal for Feed-Forward 3D Pretraining** — (arXiv:2607.02707, Jul 2026, cs.CV). Uses *frozen* vision-language features as free multi-view semantic supervision, enforcing that a point's representation stays consistent when reprojected across views — no 3D annotations needed. *Why it matters:* a modern, annotation-free instance of the generic-view / non-accidental-property prior from Ch 1–2: cross-view consistency *is* the prior $p(S)$ that regularizes an otherwise ill-posed 3D lift. [arXiv](https://arxiv.org/html/2607.02707v1)

**3. Lightweight 3D Feature Pretraining by Bayesian Inversion of 2D Foundation Models** — Hariat, Franchi, Filliat, Manzanera (ENSTA/IP Paris). (arXiv:2606.21292, Jun 2026, cs.CV). Treats a strong 2D foundation model as a *likelihood* and runs an explicit **Bayesian inversion** to distill 3D features cheaply. *Why it matters:* almost a textbook realization of Ch 1's Helmholtz/Bayes thesis — perception as inverting a forward model under a prior — now used as a pretraining recipe rather than a per-image solve. [arXiv](https://arxiv.org/abs/2606.21292)

**4. Bridging Geographic Bias in Urban Streetscape Inference via Lifelong Learning with Visual-Semantic Pivoting** — (arXiv:2606.15055, Jun 2026, cs.CV). Documents that streetscape models degrade systematically by region and proposes lifelong learning with a visual-semantic pivot to close the *per-region* performance gap. *Why it matters:* a direct, contemporary continuation of Ch 4 — disparities must be measured **per group** (here, geography) and mitigated without wrecking overall accuracy ("fairness without harm"). [arXiv](https://arxiv.org/pdf/2606.15055)

**5. Bias Is a Subspace, Not a Coordinate: A Geometric Rethinking of Post-hoc Debiasing in Vision-Language Models** — (arXiv:2511.18123, cs.CV; actively updated into 2026). Shows social bias in VLM embeddings lives in a *multi-dimensional subspace*, so single-direction "erase one coordinate" debiasing is insufficient; proposes subspace-level projection. *Why it matters:* sharpens Ch 4's mitigation discussion (adversarial/counterfactual debiasing) with a concrete geometric account of *why* naive fixes under-perform, and how to measure residual bias. [arXiv](https://arxiv.org/pdf/2511.18123)

*Note (autonomous run): truly "last-1–2-weeks" indexing is sparse for foundations-flavored work; items 1–2 are July 2026, items 3–4 are June 2026, and item 5 is a late-2025 paper under active 2026 revision — all selected for tight relevance to Day 1's inference/representation/fairness themes.*
